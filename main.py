...
def get_available_port(default_port=7860):
    preferred_ports = []
...
def process_inputs(audio_filepath, image_filepath, video_filepath):
    ...
    if not image_filepath and video_filepath:
        raise gr.Error(
            "Video-only uploads are not supported by the current vision backend. Please upload a skin image so the consultation can proceed. "
            "The current system cannot process the video directly, and the video audio is not used for transcription."
        )
    ...
    try:
        ...
        error_message = (
            ...
            "Please check your microphone upload, audio recording, and API key settings. "
            f"Details: {exc}"
        )
                ...
                with gr.Column(elem_classes="ais-card ais-input-card"):
                    ...
                    gr.HTML(
                        ...
                        <div class="ais-note">
                            ...
                            <span>For better assessment, include a short video showing the affected area from multiple angles and under good lighting. If you record audio on the video, please also upload a separate clear voice recording, because the current app does not rely on video audio.</span>
                        </div>
                        """
                    )
